...
```
_Here I used `textwrap`. This python module provides formatting of text by adjusting the line breaks in the input paragraph. The `dedent`function in this case is particularly useful to keep the code tidy without adding the actual line breaks. 

Here I have a connection string which must be indicated in the same line. If I arbitrarily just format the string with triple quotes, I would be adding line breaks (`\n`) which would break the string. _

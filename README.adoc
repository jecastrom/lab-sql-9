...
:ft-dbapi: DB-API is an acronym for DataBase Application Programming Interface and a library that lets Python connect to the database server.
...
For example in my case, I could not connect to the database using the `ODBC Driver 13 for SQL Server` as specified my Azure. I could connect successfully using the `ODBC Driver 17 for SQL Server`. Also we have to make sure that this driver is actually install in our pc.
